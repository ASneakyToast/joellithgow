/**
 * Voronoi Edge fragment shader
 * Cell boundaries creating crack or tile-like patterns
 */

import {
  baseUniformDeclarations,
  voronoiEdgeUniformDeclarations
} from '../uniforms';
import {
  commonNoiseFunctions,
  sharedMainUtilities,
  animationDirection
} from '../common.glsl';

/** Voronoi Edge (F2-F1) function */
const voronoiEdge2D = `
float voronoiEdge2D(vec2 p) {
  vec2 scaledP = p * uCellScale;
  vec2 i = floor(scaledP);
  vec2 f = fract(scaledP);

  float f1 = 1.0; // Closest
  float f2 = 1.0; // Second closest

  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      vec2 neighbor = vec2(float(x), float(y));
      vec2 point = vec2(
        hash2D(i + neighbor),
        hash2D(i + neighbor + vec2(31.0, 17.0))
      );

      if (uAnimated) {
        point = 0.5 + 0.5 * sin(uTime * uCellAnimSpeed * uSpeed + 6.2831 * point);
      }

      vec2 diff = neighbor + point - f;
      float dist = length(diff);

      if (dist < f1) {
        f2 = f1;
        f1 = dist;
      } else if (dist < f2) {
        f2 = dist;
      }
    }
  }

  float edge = f2 - f1;
  // Apply edge thickness via smoothstep
  return smoothstep(0.0, uEdgeThickness, edge);
}
`;

export const voronoiEdgeFragmentShader = `
${baseUniformDeclarations}
${voronoiEdgeUniformDeclarations}

varying vec2 vUv;

${commonNoiseFunctions}
${voronoiEdge2D}
${sharedMainUtilities}

void main() {
  vec2 uv = vUv;
  vec2 p = uv * uNoiseScale;

  ${animationDirection}

  // Voronoi Edge with directional drift
  float n = voronoiEdge2D(p + timeOffset2D * 0.3);

  // Apply gradient color
  vec3 color = getGradientColor(n);
  color = applyColorShift(color);

  // Calculate alpha with soft edges
  float alpha = n * uIntensity;
  alpha *= getEdgeFade(uv);

  // Add subtle texture variation
  alpha += getMicroTexture(uv) * uIntensity;
  alpha = clamp(alpha, 0.0, 1.0);

  gl_FragColor = vec4(color, alpha);
}
`;
